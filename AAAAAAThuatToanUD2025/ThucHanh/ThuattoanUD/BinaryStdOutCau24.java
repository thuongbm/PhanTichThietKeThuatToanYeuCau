/******************************************************************************
 *  Compilation:  javac BinaryStdOutCau24.java
 *  Execution:    java BinaryStdOutCau24
 *  Dependencies: none
 *
 *  Binary output. Writes bits, bytes, chars, ints, doubles, longs, etc.
...
public final class BinaryStdOutCau24 {
    private static BufferedOutputStream out; 

    private static int buffer;     // 8-bit buffer of bits to write out
    private static int n;          // number of bits in buffer

    static {
        out = new BufferedOutputStream(System.out); // ghi ra stdout
    }

    private BinaryStdOutCau24() { }
    ...
    // các hàm ghi public
    public static void write(boolean x) { writeBit(x); }
    public static void write(byte x)    { writeByte(x & 0xff); }
    ...
    public static void write(int x, int r) {
        ...
        if (x < 0 || x >= (1 << r)) throw new IllegalArgumentException("Illegal r-bit int = " + x);
        for (int i = 0; i < r; i++) {
    ...
    public static void write(double x) { write(Double.doubleToRawLongBits(x)); }
